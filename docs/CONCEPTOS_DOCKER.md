...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8053
EXPOSE 8053
...
| `EXPOSE 8053` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8053"]
            POSTGRES[("SERVIDOR DE BASE DE DATOS<br/>PostgreSQL · contenedor postgres<br/>hostname: postgres · escucha en 5432")]
        end
    end
    NAV -->|"localhost:8053"| APIFACTURAS
    APIFACTURAS -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15453"| POSTGRES
```
...
```yaml
  postgres:
    ...
    ports:
      - "15453:5432"                 # "puerto en su PC : puerto interno"
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8053`, `localhost:15453`); entre contenedores, nombres de
...
docker network create proyecto_diseno_de_software2_default

# 2. postgres
docker run -d --name postgres --network proyecto_diseno_de_software2_default --restart unless-stopped `
  ...
  -p 15453:5432 postgres:16-alpine
...
docker run -d --name api-facturas --network proyecto_diseno_de_software2_default --restart unless-stopped `
...
```yaml
URL_API: http://api-facturas:8053
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8053` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8053` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8053:8053`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8053`, el front
...
```powershell
docker network inspect proyecto_diseno_de_software2_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_diseno_de_software2` produce
la red `proyecto_diseno_de_software2_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_diseno_de_software2-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_diseno_de_software2_default has active endpoints (name:"proyecto_diseno_de_software2-api-facturas-1" …)
```
